...
async def test_full_flow() -> None:
    ...
    async with AsyncClient(base_url="http://localhost:8081") as proxy_client:
...
async def test_privacy_filtering() -> None:
    ...
    async with AsyncClient(base_url="http://localhost:8081") as proxy_client:
        ...
        response1 = await proxy_client.post(
            "/v1/chat/completions",
            json={
                "messages": [{"role": "user", "content": "My secret API key is sk-test-12345"}],
...
async def test_web_ui_accessible() -> None:
    ...
    async with AsyncClient(base_url="http://localhost:8081") as client:
        ...
        assert response.status_code in [200, 302, 307]  # OK or redirect
        
        # Test static file
        response = await client.get("/static/index.html")
        assert response.status_code == 200
        assert "Sekha Proxy" in response.text or "sekha" in response.text.lower()

...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            memory=MemoryConfig(
                ...
                preferred_labels=os.getenv("PREFERRED_LABELS", "").split(",") if os.getenv("PREFERRED_LABELS") else [],
                excluded_folders=os.getenv("EXCLUDED_FOLDERS", "").split(",") if os.getenv("EXCLUDED_FOLDERS") else []
            ),
            privacy=PrivacyConfig(
                exclude_from_ai_context=os.getenv("EXCLUDE_FROM_AI_CONTEXT", "false").lower() == "true"
            )
        )
    ...
    def validate(self) -> None:
        ...
        if self.llm.provider not in ["ollama", "openai", "anthropic", "google", "cohere"]:
